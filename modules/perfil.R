...
###############################################.  
# Trend help pop-up
observeEvent(input$help_perf, {
  
  showModal(modalDialog(
    title = "como utilizar esta grafica",
    p("El grafico de barra esta disenado para explorar los pozo del campo para observar su valor de vpn"),
    p(column(7,
             img(src="grafica_vpn_.png")),
      column(5,
             p("Primero seleccione un indicador usando el filtro 'paso 1'."),
             p("Seleccione el pozo de interes"))),
    size = "l", easyClose = TRUE, fade=FALSE,footer = modalButton("Close (Esc)")))
}) 



id_basica=c("Perfil","n_well","sin_fijo")
name=c("Perfil","Numero de pozo","Sin Costo Fijo")
definicion=c("se distribuye por el aporte de los fluido totales","Los Costos fijo se distribuyen por el total de pozo trabajados",
             HTML("No se tiene en cuenta los costos fijo para la evaluacion economica"))

df<-data.frame(id_basica,name,definicion)




defs_data_well <- reactive({df %>% subset(input$id_basica == id_basica)})

output$defs_text_perf <- renderUI({
  
  HTML(paste(sprintf("<b><u>%s</b></u> <br> %s ", defs_data_well()$name, 
                     defs_data_well()$definicion), collapse = "<br><br>"))
})
...
## Reactive data ----
###############################################.
#Time trend data. Filtering based on user input values.1
observe({
...
####
output$table <- renderTable({ head( table_data(), n = 11)},  
