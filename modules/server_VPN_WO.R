...
observe({
  ...
  if(input$v_dist_pozo2_1 == FALSE){
    ...
    bd_e_a <- bd_WO %>%
      dplyr::filter(campo == input$id_tipo_campo_VPNwo)
  ...
  updateSelectInput(session, "tipo_VPN_wo", choices = t_categoria)
  
  
}) 
#### pozo wo

vpn_data_wo <- reactive ({
  
  if (input$v_dist_pozo2_1 == FALSE){
    
    bd_eco <- bd_WO %>%
      dplyr::filter(campo == input$id_tipo_campo_VPNwo)%>%
      dplyr::filter(Socio == input$id_socio_VPN_WO)%>%
      dplyr::filter(E_Brent == input$id_brent_VPN_WO)%>%
      dplyr::filter(tipo_costo == input$id_tipo_costovpn_wo) %>%
      dplyr::filter(P_D == input$id_PD2_vpn_wo)
    ...
    bd_eco <-bd_WO %>%
      ...
      dplyr::filter(tipo_costo == input$id_tipo_costovpn_wo) %>%
      dplyr::filter(P_D == input$id_PD2_vpn_wo) %>%
      dplyr::filter(well == input$tipo_VPN_wo)  
...
plot_vpn_chart_wo <- function() {
  ...
  }else if (input$v_dist_pozo2_1==TRUE & input$id_FC_VPN_WO=="fc_t"){
  ...
  }else if (input$v_dist_pozo2_1==TRUE & input$id_FC_VPN_WO=="fc_d"){
    
    
    
    fig <- bd_prueba_fc %>%
      tidyr::gather(year,Valor,8:length(bd_prueba_fc)) %>%
...
output$vpn_wo1<- renderPlotly({ plot_vpn_chart_wo() })


output$table_vpn_wo1 <- DT::renderDataTable({
  
  bd_f<-data.frame(vpn_data_wo())%>%
...
output$users_vpn_wo1<- renderInfoBox({ 
...
output$users_vpn_wo2<- renderInfoBox({ 
...
output$users_vpn_wo3<- renderInfoBox({ 
