...
#####################.
#Plot 
plot_well_chart <- function() {
  
  
  if (input$v_dist_pozo==FALSE){
    
    dt<-data.frame(well_data())%>%
      dplyr::group_by(well)%>%
      dplyr::summarise(mean_vpn = quantile(VPN_Total,0.5)/1000)
...
output$vpn_pozo_basica <- renderPlotly({ plot_well_chart() }) 


output$table_vpn_t_p <- DT::renderDataTable({
...
output$users_vpn1p<- renderInfoBox({ 
...
output$users_vpn2p<- renderInfoBox({ 
...
output$users_vpn3p<- renderInfoBox({ 
